...
commands = (
    ...
    (
        "recoverable_size <size>",
        "List recoverable partitions based on the minimum <size>",
    ),
    ...
    if cmd == "help":
        ...
        if len(arguments) != 2:
            ...
            if part1.fs_type != part2.fs_type:
                print(
                    "Cannot merge partitions with types (%s, %s)"
                    % (part1.fs_type, part2.fs_type)
                )
                return
        ...
        if len(arguments) != 1:
            ...
            list_parts(
                parts,
                shorthands,
                lambda x: x.size is not None and x.size > int(arguments[0]),
            )
    elif cmd == "other":
...
def main():
    ...
    parser.add_argument("-s", "--savefile", type=str, help="path of the scan save file")
    parser.add_argument(
        "-w",
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "--outputdir",
        type=str,
        help="directory for restored contents and output files",
    )
    ...
    if args.outputdir is None:
        logging.info("No output directory specified, defaulting to recuperabit_output")
